        ...
        <section class="section">
            <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            @if ($imageThumbnailUrl)
                                                <a data-fancybox data-caption="Thumbnail" id="fancybox" href="{{ $imageThumbnailUrl }}">
                ...
                @if ($blog->status == 'draft')
                    <form id="formPublish"
                        action="{{ route('blogs.publish', ['locale' => app()->getLocale(), 'slug' => app()->getLocale() == 'en' && $blog->slug_en ? $blog->slug_en : $blog->slug]) }}"
                    ...
                    <a href="{{ route('blogs.show', ['locale' => app()->getLocale(), 'slug' => app()->getLocale() == 'en' && $blog->slug_en ? $blog->slug_en : $blog->slug]) }}"
        ...
        <script>
            $(document).ready(function() {
                $("#formEditBlog").on("submit", function(e) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: `{{ route('blogs.update', ['slug' => ':blogSlug']) }}`.replace(
                            ':blogSlug', blogSlug),
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                Swal.fire({
                                    ...
                                    window.location.href =
                                        '{{ route('blogs.show', ['locale' => app()->getLocale(), 'slug' => app()->getLocale() == 'en' && $blog->slug_en ? $blog->slug_en : $blog->slug]) }}';
                                });
                            } else {
                                if (response.errors) {
                                    var errorMessages = '';
